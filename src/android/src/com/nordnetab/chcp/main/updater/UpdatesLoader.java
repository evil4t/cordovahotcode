...
public class UpdatesLoader {
    ...
    public static ChcpError getApplicationConfigRequest(final UpdateDownloadRequest request) {
        // if download already in progress - exit
        if (isExecuting) {
            return ChcpError.DOWNLOAD_ALREADY_IN_PROGRESS;
        }
        ...
        final CheckUpdateWorker task = new CheckUpdateWorker(request);
        executeTask(task);

        return ChcpError.NONE;
    }

    public static ChcpError doUpdateRequest(final UpdateDownloadRequest request, ApplicationConfig appConfig) {
        // if download already in progress - exit
        if (isExecuting) {
            return ChcpError.DOWNLOAD_ALREADY_IN_PROGRESS;
        }
        ...
        final DoUpdateWorker task = new DoUpdateWorker(request, appConfig);
        executeTask(task);
