...
import com.nordnetab.chcp.main.network.DownLoadListener;
 ...
 * Used internally.
 */
class DoUpdateWorker implements WorkerTask {
    ...
    private FilesDownloadConfig filesDownloadConfig;
     ...
     */
    DoUpdateWorker(final UpdateDownloadRequest request, ApplicationConfig appConfig) {
    ...
    public void run() {
        ...
        final ContentManifest newContentManifest = downloadContentManifest(newAppConfig.getContentConfig().getContentUrl());
        ...
        // download files
        boolean isDownloaded = downloadNewAndChangedFiles(newAppConfig.getContentConfig().getContentUrl(), diff);
        if (!isDownloaded) {
            cleanUp();
    ...
    private boolean downloadNewAndChangedFiles(final String contentUrl, final ManifestDiff diff) {
        ...
        filesDownloadConfig = new FilesDownloadConfig(downloadFiles.size());
        setFileDownloadedNotify(filesDownloadConfig);
        EventBus.getDefault().post(resultEvent);
        try {
            FileDownloader.downloadFiles(filesStructure.getDownloadFolder(), contentUrl, downloadFiles, requestHeaders, new DownLoadListener() {
                @Override
                public void downloadfinish(int count) {
                    filesDownloadConfig.setCurrentfile(count);
                    setFileDownloadedNotify(filesDownloadConfig);
                    EventBus.getDefault().post(resultEvent);
                }
            });
    ...
    private void setFileDownloadedNotify(final FilesDownloadConfig filesDownloadConfig) {
        resultEvent = new FileDownloadedEvent(filesDownloadConfig);
    }
