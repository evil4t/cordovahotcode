...
import com.nordnetab.chcp.main.events.CheckUpdateSuccessEvent;
import com.nordnetab.chcp.main.events.FileDownloadedEvent;
...
public class HotCodePushPlugin extends CordovaPlugin {
    ...
    private FetchUpdateOptions _fetchOptions = null;
    ...
    public boolean execute(String action, CordovaArgs args, CallbackContext callbackContext) throws JSONException {
        ...
        } else if (JSAction.DO_UPDATE.equals(action)) {
            jsDoUpdate(callbackContext, args);
        } else {
            cmdProcessed = false;
        }
    ...
    private void jsDoUpdate(CallbackContext callback, CordovaArgs args) {
        ...
        String config = "";
        try {
            config = args.optJSONObject(0).get("config").toString();
        } catch (JSONException e) {
            config = args.optJSONObject(0).toString();
            e.printStackTrace();
        }
        ApplicationConfig applicationConfig = ApplicationConfig.fromJson(config);
        doUpdate(callback, applicationConfig);
    }
    ...
    private void jsCheckUpdate(CallbackContext callback, CordovaArgs args) {
        ...
        try {
            _fetchOptions = new FetchUpdateOptions(args.optJSONObject(0));
        } catch (JSONException ignored) {
        }
        checkUpdate(callback, _fetchOptions);
    }
    ...
    private void jsGetVersionInfo(final CallbackContext callback) {
        ...
        System.out.println("hahaha" + pluginResult.getMessage());
        callback.sendPluginResult(pluginResult);
    }
    ...
    private void checkUpdate(CallbackContext jsCallback, FetchUpdateOptions fetchOptions) {
        ...
        final ChcpError error = UpdatesLoader.getApplicationConfigRequest(request);
        if (error != ChcpError.NONE) {
            if (jsCallback != null) {
                PluginResult errorResult = PluginResultHelper.createPluginResult(UpdateDownloadErrorEvent.EVENT_NAME, null, error);
                jsCallback.sendPluginResult(errorResult);
    ...
    private void doUpdate(CallbackContext jsCallback, ApplicationConfig applicationConfig) {
        if (!isPluginReadyForWork) {
            return;
        }
        ...
        Map<String, String> requestHeaders = null;
        String configURL = chcpXmlConfig.getConfigUrl();
        if (_fetchOptions == null) {
            _fetchOptions = defaultFetchUpdateOptions;
        }
        if (_fetchOptions != null) {
            requestHeaders = _fetchOptions.getRequestHeaders();
            final String optionalConfigURL = _fetchOptions.getConfigURL();
            if (!TextUtils.isEmpty(optionalConfigURL)) {
                configURL = optionalConfigURL;
            }
        }
        ...
        final ChcpError error = UpdatesLoader.doUpdateRequest(request, applicationConfig);
        if (error != ChcpError.NONE) {
            if (jsCallback != null) {
                PluginResult errorResult = PluginResultHelper.createPluginResult(UpdateDownloadErrorEvent.EVENT_NAME, null, error);
                jsCallback.sendPluginResult(errorResult);
    ...
    @SuppressWarnings("unused")
    @Subscribe
    public void onEvent(CheckUpdateSuccessEvent event) {
        PluginResult jsResult = PluginResultHelper.pluginResultFromEvent(event);
        // notify JS
        if (downloadJsCallback != null) {
            downloadJsCallback.sendPluginResult(jsResult);
    ...
    public void onEvent(FileDownloadedEvent event) {
        PluginResult jsResult = PluginResultHelper.pluginResultFromEventWithKeepCallback(event);
        // notify JS
        if (downloadJsCallback != null) {
            downloadJsCallback.sendPluginResult(jsResult);
        }
