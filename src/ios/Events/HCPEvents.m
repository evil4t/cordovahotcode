...
NSString *const kHCPEventUserInfoFilesDownloadConfigKey = @"filesConfig";

NSString *const kHCPCheckUpdateSuccessEvent = @"chcp_checkUpdateSuccess";
NSString *const KHCPFileDownloaded = @"chcp_filedownloaded";
...
+ (NSNotification *)notificationWithName:(NSString *)name filesDownloadConfig:(HCPFilesDownloadConfig* )fileConfig taskId:(NSString *)taskId {
    NSMutableDictionary *userInfo = [[NSMutableDictionary alloc] init];
    if (taskId) {
        userInfo[kHCPEventUserInfoTaskIdKey] = taskId;
    }
    if (fileConfig) {
        userInfo[kHCPEventUserInfoFilesDownloadConfigKey] = fileConfig;
    }
    return [NSNotification notificationWithName:name object:nil userInfo:userInfo];
}
@end
