...
- (instancetype)initWithNewVersion:(NSString *)newVersion currentVersion:(NSString *)currentVersion {
    ...
    if (self) {
        ...
        NSLog(@"HCP_LOG : HCPInstallationWorker initWithNewVersion");
    }
...
- (void)runWithComplitionBlock:(void (^)(void))updateInstallationComplitionBlock {
    ...
    if (![self initBeforeRun:&error] ||
        ...
        ![self moveDownloadedFilesToWwwFolder:&error]) {
            NSLog(@"%@. HCP_LOG : HCPInstallationWorker Error code %ld", [error underlyingErrorLocalizedDesription], (long)error.code);
...
- (void)dispatchBeforeInstallEvent {
    NSLog(@"HCP_LOG : HCPInstallationWorker dispatchBeforeInstallEvent");
...
- (void)dispatchEventWithError:(NSError *)error {
    NSLog(@"HCP_LOG : HCPInstallationWorker dispatchEventWithError");
    NSString *message = @"HCPInstallationWorker dispatchEventWithError";
...
- (void)dispatchSuccessEvent {
    NSLog(@"HCP_LOG : HCPInstallationWorker dispatchSuccessEvent");
...
- (BOOL)initBeforeRun:(NSError **)error {
    *error = nil;
    NSLog(@"HCP_LOG : HCPInstallationWorker initBeforeRun");
    ...
    if (_oldConfig == nil) {
        *error = [NSError errorWithCode:kHCPLocalVersionOfApplicationConfigNotFoundErrorCode
        ...
        NSLog(@"HCP_LOG : Failed to load application config from cache folder");
        return NO;
    }
    ...
    if (_newConfig == nil) {
        *error = [NSError errorWithCode:kHCPLoadedVersionOfApplicationConfigNotFoundErrorCode
         ...
         NSLog(@"HCP_LOG : Failed to load application config from download folder");
        return NO;
    }
    ...
    if (_oldManifest == nil) {
        *error = [NSError errorWithCode:kHCPLocalVersionOfManifestNotFoundErrorCode
        ...
        NSLog(@"HCP_LOG : Failed to load content manifest from cache folder");
        return NO;
    }
    ...
    if (_newManifest == nil) {
        *error = [NSError errorWithCode:kHCPLoadedVersionOfManifestNotFoundErrorCode
        ...
        NSLog(@"HCP_LOG : Failed to load content manifest from download folder");
        return NO;
    }
    NSLog(@"HCP_LOG : calculate difference between the old and the new manifests");
    ...
    for (HCPManifestFile *updatedFile in updateFileList) {
        ...
        @autoreleasepool {
            ...
            if (![_fileManager fileExistsAtPath:fileLocalURL.path]) {
                ...
                NSLog(@"HCP_LOG : Update validation error! File not found: %@", updatedFile.name);
                break;
            }
            ...
            if (![fileMD5 isEqualToString:updatedFile.md5Hash]) {
                ...
                NSLog(@"HCP_LOG : Update validation error! File's %@ hash %@ doesnt match the hash %@ from manifest file", updatedFile.name, fileMD5, updatedFile.md5Hash);
                break;
            }
        }
    }
...
- (BOOL)copyFilesFromCurrentReleaseToNewRelease:(NSError **)error {
    *error = nil;
    ...
    NSLog(@"HCP_LOG : copyFilesFromCurrentReleaseToNewRelease from %@ to %@", _currentReleaseFS.wwwFolder.path, _newReleaseFS.wwwFolder.path);
...
- (BOOL)moveDownloadedFilesToWwwFolder:(NSError **)error {
    NSLog(@"HCP_LOG : HCPInstallationWorker moveDownloadedFilesToWwwFolder");
    *error = nil;
    ...
    for (HCPManifestFile *manifestFile in updatedFiles) {
        ...
        @autoreleasepool {
            ...
            if ([fileManager fileExistsAtPath:pathInWwwFolder.path] && ![fileManager removeItemAtURL:pathInWwwFolder error:error]) {
                ...
                NSLog(@"HCP_LOG : Failed to delete old version of the file %@ : %@. Installation failed",
                      manifestFile.name, [(*error) underlyingErrorLocalizedDesription]);
                break;
            }
            ...
            if (![fileManager fileExistsAtPath:parentDirectoryPathInWwwFolder.path]) {
                if (![fileManager createDirectoryAtPath:parentDirectoryPathInWwwFolder.path withIntermediateDirectories:YES attributes:nil error:error]) {
                    errorMsg = [NSString stringWithFormat:@"Failed to create folder structure for file %@ : %@. Installation failed.",
                                manifestFile.name, [(*error) underlyingErrorLocalizedDesription]];
                    NSLog(@"HCP_LOG : Failed to create folder structure for file %@ : %@. Installation failed.",
                          manifestFile.name, [(*error) underlyingErrorLocalizedDesription]);
                    break;
                }
            }
            ...
            if (![fileManager moveItemAtURL:pathInInstallationFolder toURL:pathInWwwFolder error:error]) {
                errorMsg = [NSString stringWithFormat:@"Failed to copy file %@ into www folder: %@. Installation failed.",
                            manifestFile.name, [(*error) underlyingErrorLocalizedDesription]];
                NSLog(@"HCP_LOG : Failed to copy file %@ into www folder: %@. Installation failed.",
                      manifestFile.name, [(*error) underlyingErrorLocalizedDesription]);
                break;
            }
        }
    ...
    if (errorMsg) {
        NSLog(@"HCP_LOG : kHCPFailedToCopyNewContentFilesErrorCode ");
...
- (void)saveNewConfigsToWwwFolder {
    NSLog(@"HCP_LOG : HCPInstallationWorker saveNewConfigsToWwwFolder");
...
- (void)cleanUpOnFailure {
    NSLog(@"HCP_LOG : HCPInstallationWorker cleanUpOnFailure");
...
- (void)cleanUpOnSucess {
    NSLog(@"HCP_LOG : HCPInstallationWorker cleanUpOnSucess");
