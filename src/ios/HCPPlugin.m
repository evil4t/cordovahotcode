...
@interface HCPPlugin() {
    ...
    HCPFetchUpdateOptions *_options;
}
...
- (void)onResume:(NSNotification *)notification {
    ...
    NSLog(@"HCP_LOG : load app config from update folder and check, if we are allowed to install it");
...
- (void)installWwwFolder {
    ...
    if (_pluginInternalPrefs.isWwwFolderInstalled) {
        NSLog(@"HCP_LOG : reset www folder installed flag.");
...
- (void)doLocalInit {
    ...
    NSLog(@"HCP_LOG : Currently running release version %@", _pluginInternalPrefs.currentReleaseVersionName);
...
- (void)doFileLogInit {
    ...
    if ([fm fileExistsAtPath:filePath]) {
        ...
        if (!fileAttributes || !(creatDate = [fileAttributes objectForKey:NSFileCreationDate])) {
            NSLog(@"HCP_LOG debug log error");
            [fm removeItemAtPath:filePath error:nil];
            [fm createFileAtPath:filePath contents:nil attributes:nil];
        } else {
           long createDateValue = (long)[creatDate timeIntervalSince1970]*1000;
            NSLog(@"HCP_LOG date = %@",creatDate);
            if ([self getNowTimeTimestamp] - createDateValue > 604800000) { // 7天
                NSLog(@"HCP_LOG debug log time out delete");
                [fm removeItemAtPath:filePath error:nil];
                [fm createFileAtPath:filePath contents:nil attributes:nil];
            } else {
                NSString *s = [NSString stringWithFormat:@"========== %@ =========", creatDate];
                [self writeFileLog:s];
            }
        }
    } else {
        NSLog(@"HCP_LOG create debug log");
        [fm createFileAtPath:filePath contents:nil attributes:nil];
    }
}

- (void)writeFileLog:(NSString *)message {
    NSString *m = [NSString stringWithFormat:@"%@\n", message];
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory,NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0];
    NSString *filePath = [documentsDirectory stringByAppendingPathComponent:@"Debug.log"];
    NSData *contentData = [m dataUsingEncoding:NSUnicodeStringEncoding];
    NSFileHandle *fileHandle = [NSFileHandle fileHandleForUpdatingAtPath:filePath];
    [fileHandle seekToEndOfFile];
    [fileHandle writeData:contentData]; //追加写入数据
    [fileHandle closeFile];
    NSLog(@"HCP_LOG %@ >>write ok.", message);
}
...
- (BOOL)_fetchUpdate:(NSString *)callbackId withOptions:(HCPFetchUpdateOptions *)options {
    if (!_isPluginReadyForWork) {
        NSLog(@"HCP_LOG : PluginReadyForWork is false");
        return NO;
    }
    ...
    NSLog(@"HCP_LOG : fetchUpdate init configURL %@ %@ %ld", [request.configURL absoluteString], request.currentWebVersion, request.currentNativeVersion);
    
    NSError *error = nil;
    [[HCPUpdateLoader sharedInstance] executeDownloadRequest:request error:&error];
    
    if (error) {
        NSLog(@"HCP_LOG : fetchUpdate executeDownloadRequest error %@", [error localizedDescription]);
        if (callbackId) {
            NSLog(@"HCP_LOG : fetchUpdate executeDownloadRequest error callbackId %@", callbackId);
...
- (BOOL)_installUpdate:(NSString *)callbackID {
    ...
    NSLog(@"HCP_LOG : installUpdate newVersion %@ currentVersion %@ ", newVersion, currentVersion);
    NSError *error = nil;
    [[HCPUpdateInstaller sharedInstance] installVersion:newVersion currentVersion:currentVersion error:&error];
    if (error) {
        NSLog(@"HCP_LOG : installUpdate HCPUpdateInstaller error %@", [error localizedDescription]);
        if (error.code == kHCPNothingToInstallErrorCode) {
            ...
            if (callbackID) {
                NSString *message = [NSString stringWithFormat:@"installUpdate HCPUpdateInstaller error %@", [error localizedDescription]];
                [self writeFileLog:message];
...
- (BOOL)_checkUpdate:(NSString *)callbackId withOptions:(HCPFetchUpdateOptions *)options {
    if (!_isPluginReadyForWork) {
    ...
    NSLog(@"HCP_LOG : checkUpdate init configURL %@ %@ %ld", [request.configURL absoluteString], request.currentWebVersion, request.currentNativeVersion);
    
    NSError *error = nil;
    [[HCPUpdateLoader sharedInstance] executeApplicationConfigRequest:request error:&error];
    
    if (error) {
        NSLog(@"HCP_LOG : checkUpdate executeDownloadRequest error %@", [error localizedDescription]);
        if (callbackId) {
            NSLog(@"HCP_LOG : checkUpdate executeDownloadRequest error callbackId %@", callbackId);
            CDVPluginResult *errorResult = [CDVPluginResult pluginResultWithActionName:kHCPUpdateDownloadErrorEvent
                                                                     applicationConfig:nil
                                                                                 error:error];
            [self.commandDelegate sendPluginResult:errorResult callbackId:callbackId];
    ...
    if (callbackId) {
        _downloadCallback = callbackId;
        NSLog(@"HCP_LOG : checkUpdate _downloadCallback %@", callbackId);
    }
    
    return YES;
}

- (BOOL)_doUpdate:(NSString *)callbackId withOptions:(HCPApplicationConfig *)newAppConfig {
    if (!_isPluginReadyForWork) {
        return NO;
    }
    
    if (!_options && self.defaultFetchUpdateOptions) {
        _options = self.defaultFetchUpdateOptions;
    }
    
    HCPUpdateRequest *request = [[HCPUpdateRequest alloc] init];
    request.configURL = _options.configFileURL ? _options.configFileURL : _pluginXmlConfig.configUrl;
    request.requestHeaders = _options.requestHeaders;
    request.currentWebVersion = _pluginInternalPrefs.currentReleaseVersionName;
    request.currentNativeVersion = _pluginXmlConfig.nativeInterfaceVersion;
    
    NSLog(@"HCP_LOG : _doUpdate init configURL %@ %@ %ld", [request.configURL absoluteString], request.currentWebVersion, request.currentNativeVersion);
    
    NSError *error = nil;
    [[HCPUpdateLoader sharedInstance] executeUpdateRequest:request options:newAppConfig error:&error];
    
    if (error) {
        NSLog(@"HCP_LOG : _doUpdate executeDownloadRequest error %@", [error localizedDescription]);
        if (callbackId) {
            NSLog(@"HCP_LOG : _doUpdate executeDownloadRequest error callbackId %@", callbackId);
            CDVPluginResult *errorResult = [CDVPluginResult pluginResultWithActionName:kHCPUpdateDownloadErrorEvent
                                                                     applicationConfig:nil
                                                                                 error:error];
            [self.commandDelegate sendPluginResult:errorResult callbackId:callbackId];
    ...
    if (callbackId) {
        _downloadCallback = callbackId;
        NSLog(@"HCP_LOG : _doUpdate _downloadCallback %@", callbackId);
    }
...
- (void)_retryDownload {
    NSLog(@"HCP_LOG : _retryDownload");
    NSNumber *needRetryNum = [NSNumber numberWithInteger:1];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"xxxxxxxxxxNeedRetry" object:needRetryNum];
}

- (void)_cancelDownload {
    NSLog(@"HCP_LOG : _cancelDownload");
    NSNumber *needRetryNum = [NSNumber numberWithInteger:0];
    [[NSNotificationCenter defaultCenter] postNotificationName:@"xxxxxxxxxxNeedRetry" object:needRetryNum];
}
...
- (void)resetIndexPageToExternalStorage {
    NSLog(@"HCP_LOG : resetIndexPageToExternalStorage");
    ...
    if ([self.viewController isKindOfClass:[CDVViewController class]]) {
        ...
        NSLog(@"HCP_LOG :  HotCodePushError: Can't make starting page to be from external storage. Main controller should be of type CDVViewController.");
    }
}
...
- (NSURL *)appendWwwFolderPathToPath:(NSString *)pagePath {
    NSLog(@"HCP_LOG : appendWwwFolderPathToPath %@", pagePath);
...
- (void)subscribeToPluginInternalEvents {
    ...
    [notificationCenter addObserver:self
                           selector:@selector(onCheckUpdateSuccess:)
                               name:kHCPCheckUpdateSuccessEvent
                             object:nil];
    [notificationCenter addObserver:self
                           selector:@selector(onGetDownloadFilesCount:)
                               name:KHCPFileDownloaded
                             object:nil];
}
...
- (void)onBeforeAssetsInstalledOnExternalStorageEvent:(NSNotification *)notification {
    NSLog(@"HCP_LOG : onBeforeAssetsInstalledOnExternalStorageEvent");
...
- (void)onAssetsInstallationErrorEvent:(NSNotification *)notification {
    _isPluginReadyForWork = NO;
    NSLog(@"HCP_LOG : onAssetsInstallationErrorEvent");
    // send notification to web
    [self invokeDefaultCallbackWithMessage:[CDVPluginResult pluginResultForNotification:notification]];
}
...
- (void)onUpdateDownloadErrorEvent:(NSNotification *)notification {
    ...
    NSLog(@"HCP_LOG : Error during update: %@", [error underlyingErrorLocalizedDesription]);
    NSString *message =  [NSString stringWithFormat:@"Error during update: %@", [error underlyingErrorLocalizedDesription]];
    [self writeFileLog:message];
...
- (void)onNothingToUpdateEvent:(NSNotification *)notification {
    NSLog(@"HCP_LOG : Nothing to update");
...
- (void)onUpdateIsReadyForInstallation:(NSNotification *)notification {
    ...
    NSLog(@"HCP_LOG : Update is ready for installation: %@", newConfig.contentConfig.releaseVersion);
    ...
    if (_pluginXmlConfig.isUpdatesAutoInstallationAllowed &&
        ...
        ![HCPUpdateLoader sharedInstance].isDownloadInProgress &&
        ![HCPUpdateInstaller sharedInstance].isInstallationInProgress) {
        NSLog(@"HCP_LOG : Update is ready for installation launch the installation");
        [self _installUpdate:nil];
    }
}

- (void)onCheckUpdateSuccess:(NSNotification *)notification {
    // new application config from server
    NSLog(@"HCP_LOG : onCheckUpdateSuccess");
    // send notification to the associated callback
    CDVPluginResult *pluginResult = [CDVPluginResult pluginResultForNotification:notification];
    if (_downloadCallback) {
        [self.commandDelegate sendPluginResult:pluginResult callbackId:_downloadCallback];
...
- (void)onGetDownloadFilesCount:(NSNotification *)notification {
    NSLog(@"HCP_LOG : onGetDownloadFilesCount");
    // send notification to the associated callback
    CDVPluginResult *pluginResult = [CDVPluginResult pluginResultForFileNotification:notification];
    pluginResult.keepCallback = @YES;
    if (_downloadCallback) {
        [self.commandDelegate sendPluginResult:pluginResult callbackId:_downloadCallback];
    }
...
- (void)onNothingToInstallEvent:(NSNotification *)notification {
    CDVPluginResult *pluginResult = [CDVPluginResult pluginResultForNotification:notification];
    NSLog(@"HCP_LOG : onNothingToInstallEvent");
...
- (void)onBeforeInstallEvent:(NSNotification *)notification {
    CDVPluginResult *pluginResult = [CDVPluginResult pluginResultForNotification:notification];
    NSLog(@"HCP_LOG : onBeforeInstallEvent");
    // send notification to the default callback
    [self invokeDefaultCallbackWithMessage:pluginResult];
}
...
- (void)onUpdateInstallationErrorEvent:(NSNotification *)notification {
    _pluginInternalPrefs.readyForInstallationReleaseVersionName = @"";
    [_pluginInternalPrefs saveToUserDefaults];
    NSLog(@"HCP_LOG : onUpdateInstallationErrorEvent");
    [self writeFileLog:@"onUpdateInstallationErrorEvent"];
...
- (void)onUpdateInstalledEvent:(NSNotification *)notification {
    ...
    NSLog(@"HCP_LOG : onUpdateInstalledEvent");
    _pluginInternalPrefs.readyForInstallationReleaseVersionName = @"";
...
- (void)rollbackToPreviousRelease {
    NSLog(@"HCP_LOG : rollbackToPreviousRelease");
    _pluginInternalPrefs.readyForInstallationReleaseVersionName = @"";
...
- (void)rollbackIfCorrupted:(NSError *)error {
    [self writeFileLog:@"rollbackIfCorrupted"];
    NSLog(@"HCP_LOG : rollbackIfCorrupted");
    ...
    if (_pluginInternalPrefs.previousReleaseVersionName.length > 0) {
        NSString *message =  [NSString stringWithFormat:@"WWW folder is corrupted, rolling back to previous version. %@", _pluginInternalPrefs.previousReleaseVersionName];
        [self writeFileLog:message];
        NSLog(@"HCP_LOG : WWW folder is corrupted, rolling back to previous version. %@", _pluginInternalPrefs.previousReleaseVersionName);
        [self rollbackToPreviousRelease];
    } else {
        [self writeFileLog:@"WWW folder is corrupted, reinstalling it from bundle."];
        NSLog(@"HCP_LOG : WWW folder is corrupted, reinstalling it from bundle.");
        [self installWwwFolder];
    }
}
...
- (void)jsInstallUpdate:(CDVInvokedUrlCommand *)command {
    if (!_isPluginReadyForWork) {
        NSString *message = @"jsInstallUpdate _isPluginReadyForWork false";
        [self writeFileLog:message];
...
- (void)jsCheckUpdate:(CDVInvokedUrlCommand *)command {
    NSLog(@"HCP_LOG : jsCheckUpdate");
    if (!_isPluginReadyForWork) {
        [self sendPluginNotReadyToWorkMessageForEvent:kHCPUpdateDownloadErrorEvent callbackID:command.callbackId];
    }
    
    NSDictionary *optionsFromJS = command.arguments.count ? command.arguments[0] : nil;
    _options = [[HCPFetchUpdateOptions alloc] initWithDictionary:optionsFromJS];

    [self _checkUpdate:command.callbackId withOptions:_options];
}
    
- (void)jsDoUpdate:(CDVInvokedUrlCommand *)command {
    if (!_isPluginReadyForWork) {
        [self sendPluginNotReadyToWorkMessageForEvent:kHCPUpdateDownloadErrorEvent callbackID:command.callbackId];
    }
    NSDictionary *optionsFromJS = command.arguments.count ? command.arguments[0] : nil;
    NSDictionary *config = optionsFromJS[@"config"];
    HCPApplicationConfig *newAppConfig = [HCPApplicationConfig instanceFromJsonObject:config];
    [self _doUpdate:command.callbackId withOptions:newAppConfig];
}

- (void)jsRetryDownload:(CDVInvokedUrlCommand *)command {
    if (!_isPluginReadyForWork) {
        return;
    }
    [self _retryDownload];
...
- (void)jsCancelDownload:(CDVInvokedUrlCommand *)command {
    if (!_isPluginReadyForWork) {
        return;
    }
