...
- (instancetype)initWithFiles:(NSArray *)filesList srcDirURL:(NSURL *)contentURL dstDirURL:(NSURL *)folderURL requestHeaders:(NSDictionary *)headers {
    ...
    if (self) {
        ...
        _headers = headers;
        _isRetry = false;
        NSLog(@"HCP_LOG : HCPFileDownloader initWithFiles folderURL %@ contentURL %@", [_folderURL absoluteString], [_contentURL absoluteString]);
    }
...
- (void)startDownloadWithCompletionBlock:(HCPFileDownloadCompletionBlock)block {
    ...
    _downloadCounter = 0;
    NSLog(@"HCP_LOG : startDownloadWithCompletionBlock _downloadCounter %ld", _downloadCounter);
    _session = [self sessionWithHeaders:_headers];
    
    [self launchDownloadTaskForFile:_filesList[0]];
}

- (void)retryDownloadWithCompletionBlock {
    _session = [self sessionWithHeaders:_headers];
    _isRetry = true;
    NSLog(@"HCP_LOG : retryDownloadWithCompletionBlock _downloadCounter %ld", _downloadCounter);
...
- (void)URLSession:(NSURLSession *)session didBecomeInvalidWithError:(NSError *)error {
    NSLog(@"HCP_LOG : didBecomeInvalidWithError : %@", [error localizedDescription]);
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    NSLog(@"HCP_LOG : didCompleteWithError _downloadCounter : %@ %ld", [error localizedDescription], _downloadCounter);
    if (error && _complitionHandler) {
        NSLog(@"HCP_LOG : _complitionHandler not null");
        _complitionHandler(error);
        _session = nil;
    }
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    NSLog(@"HCP_LOG : didFinishDownloadingToURL _downloadCounter : %@ %ld", [location absoluteString], _downloadCounter);
    NSError *error = nil;
    if (![self moveLoadedFile:location forFile:_filesList[_downloadCounter] toFolder:_folderURL error:&error]) {
        [_session invalidateAndCancel];
        _session = nil;
        _complitionHandler(error);
        NSLog(@"HCP_LOG : moveLoadedFile fail %@",  [error localizedDescription]);
        return;
    }
    
    _downloadCounter++;
    NSNumber *counter = @(_downloadCounter);
    NSLog(@"HCP_LOG : _complitionHandler _downloadCounter %ld", _downloadCounter);
    _complitionHandler(counter);
    if (_downloadCounter >= _filesList.count) {
        [_session finishTasksAndInvalidate];
        _session = nil;
        _complitionHandler(nil);
        NSLog(@"HCP_LOG : complited");
        return;
    }
...
- (void)launchDownloadTaskForFile:(HCPManifestFile *)file {
    ...
    NSLog(@"HCP_LOG : Starting file download: %@", [url absoluteString]);
...
- (BOOL)moveLoadedFile:(NSURL *)loadedFile forFile:(HCPManifestFile *)file toFolder:(NSURL *)folderURL error:(NSError **)error {
    if ([self isFileCorrupted:loadedFile checksum:file.md5Hash]) {
        ...
        NSLog(@"HCP_LOG : File %@ is corrupted --> kHCPFailedToDownloadUpdateFilesErrorCode", file.name);
    ...
    if ([fileManager fileExistsAtPath:filePath.path]) {
        NSLog(@"HCP_LOG : remove old version of the file %@", filePath.path);
    ...
    [fileManager createDirectoryAtPath:[filePath.path stringByDeletingLastPathComponent]
           withIntermediateDirectories:YES
                            attributes:nil
                                 error:nil];
    NSLog(@"HCP_LOG : create storage directories and write data");
    // write data
