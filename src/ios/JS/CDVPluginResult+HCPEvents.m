...
static NSString *const DATA_USER_INFO_COUNTER = @"counter";
...
+ (CDVPluginResult *)pluginResultForFileNotification:(NSNotification *)notification {
    HCPFilesDownloadConfig *fileConfig = notification.userInfo[kHCPEventUserInfoFilesDownloadConfigKey];
    NSError *error = notification.userInfo[kHCPEventUserInfoErrorKey];
    NSString *action = notification.name;
    
    return [CDVPluginResult pluginResultWithActionName:action filesDownloadConfig:fileConfig error:error];
}
...
+ (CDVPluginResult *)pluginResultWithActionName:(NSString *)action filesDownloadConfig:(HCPFilesDownloadConfig *)filesConfig error:(NSError *)error {
    NSDictionary *data = nil;
    if (filesConfig) {
        data = @{DATA_USER_INFO_COUNTER: [filesConfig toJson]};
    }
    
    return [self pluginResultWithActionName:action data:data error:error];
}

+ (CDVPluginResult *)pluginResultWithActionName:(NSString *)action object:(id)object error:(NSError *)error {
    NSDictionary *data = nil;
    if (object) {
        data = object;
    }
